...
const buyer={variantId:'v13-128',maxPriceCents:32000,startsOn:'2026-08-25',expiresOn:'2026-09-02',minBatteryPercent:80,requireIntactDisplay:true,requireBiometrics:true};const gate={possessionStatus:'VERIFIED',marketState:'MARKET_ELIGIBLE',provenance:'TRUSTED_SERVER'};
...
test('G2-NEG-EXPIRED-BUYER rejects intent expired before as-of',()=>{const expired={...buyer,startsOn:'2026-08-20',expiresOn:'2026-08-24'};const d=evaluateCandidate(R(),expired,gate,'2026-08-25');assert.ok(d.reasons.includes('BUYER_INTENT_EXPIRED'));assert.equal(d.eligible,false)});
...
test('G3-NEG-OPERATOR-GATE never promotes owner attestation',()=>{const d=evaluateCandidate(R(),buyer,{possessionStatus:'UNVERIFIED',marketState:'PRIVATE',provenance:'TRUSTED_SERVER'},'2026-08-25');assert.deepEqual(d.reasons,['OPERATOR_GATE_NOT_MET'])});
test('G3-NEG-UNTRUSTED-AUTHORITY rejects fabricated privileged strings',()=>{const d=evaluateCandidate(R(),buyer,{possessionStatus:'VERIFIED',marketState:'MARKET_ELIGIBLE'},'2026-08-25');assert.deepEqual(d.reasons,['OPERATOR_GATE_NOT_MET']);assert.equal(d.eligible,false)});
...
test('privacy and authority guard rejects sensitive/status fields',()=>{for(const field of ['imei','serialNumber','password','fullAddress','latitude','marketState','possessionStatus','verified','provenance'])assert.ok(codes(validateSmartphoneIntake(valid({[field]:'x'}),submitted)).includes('SENSITIVE_OR_AUTHORITY_FIELD_FORBIDDEN'))});
