...
for (const expected of [
  ...
  "'VALUE_VISIBLE'",
  "'SELL_INITIATED'",
  'snapshot.counts[ORDER[index]] < 1',
  'snapshot.counts[name] > 0',
  'recordLocalActivation(name)',
]) {
  assert.ok(source.includes(expected), `missing ordered/deduplicated bridge contract: ${expected}`);
}

for (const forbidden of ['fetch(', 'AsyncStorage', 'supabase', 'userId', 'itemId', 'price', 'timestamp', 'location']) {
  assert.ok(!source.includes(forbidden), `activation action bridge must stay process-local/privacy-minimal: ${forbidden}`);
}
