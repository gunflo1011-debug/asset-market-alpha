...
if (!listingPanel.includes("published ? 'New marketplace price (€)' : 'Seller asking price (€)'") || !listingPanel.includes('reference only · never auto-published')) {
  throw new Error('Authenticated sell convergence must keep seller asking price explicit and personal estimate reference-only.');
}
if (!listingPanel.includes('THINGS MARKET VALUE') || !listingPanel.includes('Use price') || !listingPanel.includes('setPrice(String(suggestedMarketPrice / 100))')) {
  throw new Error('Authenticated sell convergence must keep Market Value as an explicit seller-controlled suggestion.');
}
if (!listingPanel.includes('Market Value and Personal Estimate are references only and are never copied into the listing automatically.')) {
  throw new Error('Authenticated sell convergence must not auto-copy market guidance into seller asking price.');
}
...
if (!listingPanel.includes("const [price, setPrice] = useState('')")) {
  throw new Error('Authenticated sell convergence must not inherit any estimate or market suggestion into seller asking price.');
}
