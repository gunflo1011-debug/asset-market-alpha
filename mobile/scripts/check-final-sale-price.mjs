...
assert.match(conversation, /status === 'RESERVED'[\s\S]*actual final sale price/i);
assert.match(conversation, /status === 'SOLD'[\s\S]*accepted offer[\s\S]*offer history[\s\S]*confirmed final price[\s\S]*amount actually paid/i);
