...
export function MarketplaceConversationScreen({ conversation, title, onBack }: Props) {
  ...
  useEffect(() => { setStatus(conversation.status); setAdoptedItemId(null); void refresh(); }, [conversation.conversation_id, conversation.status]);
  ...
  async function changeLifecycle(nextStatus: 'RESERVED' | 'SOLD') {
    try {
      ...
      setStatus(await setMyMarketplaceConversationStatus(conversation.conversation_id, nextStatus));
  ...
  function confirmSold() {
    Alert.alert('Mark this Thing as sold?', 'Use this only after you have actually completed the sale. This closes messaging for this transaction and marks the Thing sold in your lifecycle.', [
      { text: 'Cancel', style: 'cancel' }, { text: 'Mark sold', style: 'destructive', onPress: () => void changeLifecycle('SOLD') },
    ]);
  }
  ...
  return (
    <SafeAreaView style={styles.safe}><View style={styles.container}>
      ...
      {seller && status === 'RESERVED' ? <View style={styles.lifecycleCard}><Text style={styles.lifecycleTitle}>Reserved for this buyer</Text><Text style={styles.copy}>The public listing is withdrawn. Keep using this chat to coordinate. Mark sold only after the real handover or sale is complete.</Text><TouchableOpacity disabled={lifecycleBusy} style={[styles.soldButton, lifecycleBusy && styles.disabled]} onPress={confirmSold}><Text style={styles.soldButtonText}>{lifecycleBusy ? 'Updating…' : 'Mark as sold'}</Text></TouchableOpacity></View> : null}
