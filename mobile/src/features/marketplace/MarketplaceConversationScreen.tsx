...
export function MarketplaceConversationScreen({ conversation, title, onBack }: Props) {
  ...
  const [confirmedFinalSalePriceCents, setConfirmedFinalSalePriceCents] = useState<number | null>(conversation.final_sale_price_cents ?? null);
    ...
    <SafeAreaView style={styles.safe}>
      <KeyboardAvoidingView style={styles.flex} behavior={Platform.OS === 'ios' ? 'padding' : undefined}>
        <View style={styles.container}>
          <View style={styles.header}><TouchableOpacity accessibilityRole="button" accessibilityLabel="Back to Marketplace" hitSlop={8} onPress={onBack}><Text style={styles.back}>‹ Marketplace</Text></TouchableOpacity><TouchableOpacity accessibilityRole="button" accessibilityLabel="Refresh conversation" hitSlop={8} disabled={loading} onPress={() => void refresh()}><Text style={styles.refresh}>{loading ? '…' : '↻'}</Text></TouchableOpacity></View>
          ...
          {buyer && status === 'SOLD' ? <View style={[styles.adoptionCard, adoptedItemId ? styles.adoptionSuccessCard : null]} accessibilityLabel={adoptedItemId ? 'Purchase saved privately in My Things' : `Sale complete${confirmedFinalSalePriceCents != null ? `. Paid price ${euro(confirmedFinalSalePriceCents)}` : ''}. Add this purchased Thing to My Things`}>
            <Text style={adoptedItemId ? styles.successEyebrow : styles.adoptionEyebrow}>{adoptedItemId ? 'PURCHASE SAVED' : 'SALE COMPLETE'}</Text>
            <Text style={adoptedItemId ? styles.successTitle : styles.adoptionTitle}>{adoptedItemId ? 'Added to My Things' : 'Your purchase is complete'}</Text>
            {!adoptedItemId && confirmedFinalSalePriceCents != null ? <View style={styles.paidPriceCard}><Text style={styles.paidPriceLabel}>PAID PRICE</Text><Text style={styles.paidPriceValue}>{euro(confirmedFinalSalePriceCents)}</Text><Text style={styles.paidPriceHelp}>Confirmed final sale price. This is separate from the seller’s asking price and any earlier offer.</Text></View> : null}
            <Text style={styles.adoptionCopy}>{adoptedItemId ? 'This purchased Thing is now part of your private inventory. You can open it in My Things and add your own photos, notes and details.' : 'The seller has confirmed the sale is complete. Save a private copy in My Things so the item becomes part of your inventory.'}</Text>
            <View style={styles.adoptionPrivacy}><Text style={styles.adoptionPrivacyLabel}>PRIVATE BY DEFAULT</Text><Text style={styles.adoptionPrivacyText}>Seller notes, private location, serial data, photos and account details are never copied into your Thing.</Text></View>
            {adoptedItemId ? (
              ...
              <TouchableOpacity accessibilityRole="button" accessibilityHint="Creates a new private Thing from the completed purchase" disabled={adoptionBusy} style={[styles.adoptionButton, adoptionBusy && styles.disabled]} onPress={() => void adoptPurchasedThing()}><Text style={styles.adoptionButtonText}>{adoptionBusy ? 'Adding…' : 'Add purchase to My Things'}</Text></TouchableOpacity>
            )}
