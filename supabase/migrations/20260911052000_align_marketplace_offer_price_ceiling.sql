...
begin
  if exists (
    select 1
    from private.marketplace_offers
    where amount_cents > 1000000000
  ) then
    raise exception 'MARKETPLACE_OFFER_PRICE_CEILING_PRECHECK_FAILED';
  end if;
end;
$$;

alter table private.marketplace_offers
  drop constraint marketplace_offers_amount_cents_check;

alter table private.marketplace_offers
  add constraint marketplace_offers_amount_cents_check
  check (amount_cents >= 1 and amount_cents <= 1000000000);
...
begin
  ...
  if p_amount_cents is null or p_amount_cents < 1 or p_amount_cents > 1000000000 then
...
begin
  ...
  if p_action = 'COUNTER' then
    if p_counter_amount_cents is null or p_counter_amount_cents < 1 or p_counter_amount_cents > 1000000000 then
      raise exception 'INVALID_OFFER_AMOUNT';
    end if;
